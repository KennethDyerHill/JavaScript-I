...
const car33 = inventory.filter(x => x.car_make, x.car_model, x.car_year);

console.log(`Car 33 is a ${x.car_year} ${x.car_make} ${x.car_model}`);
...
for (i=0; i<carYears.length; i++){
    if (carYears[i] <= 2000){
    oldCars.push(inventory.car_year);
};
};
console.log(oldCars);
